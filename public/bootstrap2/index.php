...
$requete1 = "UPDATE T_COMPTE_cmp SET  cmp_mote_de_passe ='".$password."' WHERE
cmp_e_mail ='leroy.david@gmail.com';";
echo($requete1);
...
<?php if (!empty($documents)): ?>
    <?php foreach ($documents as $document): ?>
        ...
        $chemin_fichier = '/public_html/gabarit/documents/' . $nom_fichier;
        $extension = pathinfo($chemin_fichier, PATHINFO_EXTENSION);
        ?>
        <div class="mb-3">
            <?php if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                <img src="<?= $chemin_fichier ?>" alt="Image téléchargée" style="max-width: 200px;"><br>
            <?php else: ?>
                <a href="<?= $chemin_fichier ?>" target="_blank"><?= $nom_fichier ?></a><br>
            <?php endif; ?>
            <p><?= esc($document['doc_description'] ?? 'Aucune description') ?></p>
        </div>
        <hr> 
    <?php endforeach; ?>
<?php else: ?>
...
// select toutes les documents dans la tables
$requete = "SELECT dcm_nom FROM T_DOCUMENT_dcm";
...
while ($row = $resultat->fetch_assoc()) {
    $nom_fichier = $row['dcm_nom'];
    $chemin_fichier = 'documents/' . $nom_fichier;
    
    $extension = pathinfo($chemin_fichier, PATHINFO_EXTENSION);
    if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif'])) {
        echo "<img src='$chemin_fichier' alt='Image téléchargée' style='max-width: 200px;'><br>";
    } else {
        echo "<a href='$chemin_fichier'>$nom_fichier</a><br>";
    }
}
